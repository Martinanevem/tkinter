...
if user != "Vendég":
    jelenlegi_pont_akasztofa = database.child('Kigyos_jatek').get().val()

    osszes = database.child('Kigyos_jatek_OSSZES').child(user).get().val()
    if osszes is None:
        osszes = 0
    else:
        osszes = int(osszes)
    
    nyert_ = database.child('Kigyos_jatek').child(user).get().val()
    if nyert_ is None:
        nyert_ = 0
    else:
        nyert_ = int(nyert_)
    ...
    for index, (key, value) in enumerate(sorted_items):
        if key == search_key:
            hely = index
    szoveg = f"{hely+1}. helyen állsz a snake ranglistán!"

    osszes = f"Összes játékok: {osszes}"
    nyert = f"Megszerzett pontok: {nyert}"
